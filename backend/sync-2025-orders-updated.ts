...
/**
 * This script syncs orders using LastUpdatedAfter instead of CreatedAfter
 * to catch orders that were created before the sync period but updated during it.
 *
 * This helps capture:
 * - Orders that changed status
 * - Orders with price updates
 * - Orders that "slipped through the cracks"
 */
async function sync2025OrdersWithLastUpdated() {
  ...
  console.log('🔄 Syncing ALL 2025 orders using LastUpdatedAfter (to catch missing orders)');
  console.log(`📅 Period: January 1, 2025 to ${today.toISOString().split('T')[0]}`);
  console.log(`⏱️  Days to sync: ${daysSinceJan1}`);
  console.log(`📝 Using: LastUpdatedAfter/LastUpdatedBefore instead of CreatedAfter/CreatedBefore\n`);

  // Sync Grifos first (has more missing orders - ~400)
  console.log('=' .repeat(60));
  console.log('🏢 GRIFOS - LastUpdated Sync');
  console.log('=' .repeat(60));
  ...
  try {
    ...
    const grifosResult = await grifosService.syncOrders(
      GRIFOS_ACCOUNT_ID,
      daysSinceJan1,
      GRIFOS_SELLER_ID,
      true  // useLastUpdated = true
    );
  ...
  // Sync Valenti
  console.log('\n' + '='.repeat(60));
  console.log('🏢 VALENTI - LastUpdated Sync');
  console.log('='.repeat(60));
  ...
  try {
    ...
    const valentiResult = await valentiService.syncOrders(
      VALENTI_ACCOUNT_ID,
      daysSinceJan1,
      VALENTI_SELLER_ID,
      true  // useLastUpdated = true
  ...
  console.log('\n' + '='.repeat(60));
  console.log('🎉 2025 LastUpdated Sync Completed!');
  console.log('💡 This sync catches orders that were updated after creation');
  console.log('='.repeat(60));
}

sync2025OrdersWithLastUpdated();
