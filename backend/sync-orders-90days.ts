// Sync recent orders (90 days) to quickly verify the fix
...
async function main() {
  ...
  console.log('=== SYNCING RECENT ORDERS (90 DAYS) ===\n');

  try {
    const orderService = new OrderService(accountId);
    // Sync last 90 days instead of 730
    const result = await orderService.syncOrders(accountId, 90);
